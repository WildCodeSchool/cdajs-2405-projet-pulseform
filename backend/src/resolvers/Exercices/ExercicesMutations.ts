...
import { CreateExerciceInput, UpdateExerciceInput } from "../../inputs/ExercicesInput";
...
export class ExercicesMutations {
    ...
    async createExercice(
        ...
        const { name, muscle, level, duration, kcal_loss, img_src, description } = data;

        // Sauvegarder l'exercice dans la base de données
        const exercice = new Exercice( name, description || "", duration, kcal_loss, muscle, level, img_src || "");
